...
const statusDisplay: Record<string, { label: string; className: string }> = {
  open: {
    label: "Open",
    className: "bg-blue-400/15 text-blue-300 border-blue-400/25",
  },
  claimable: {
    label: "Claimable",
  ...
  lost: {
    label: "Lost",
    className: "bg-red-400/15 text-red-300 border-red-400/25",
  },
}
...
export default function PortfolioPanel() {
  ...
  const totalPotential = positions.reduce(
    (sum, p) => sum + p.estimatedPayoutSol,
    0,
  )
  ...
  const settleableTotal = settleablePositions.reduce(
    (sum, p) =>
      sum + (p.status === "claimable" ? p.estimatedPayoutSol : p.amountSol),
    0,
  )
    ...
    <section aria-labelledby="portfolio-heading">
      ...
      {!connected ? (
        <div className="mt-5 rounded-2xl border border-dashed border-white/20 bg-white/[0.03] px-5 py-8 text-center">
          <div
            className="mx-auto grid size-10 place-items-center rounded-full border border-white/20 text-lg"
            aria-hidden="true"
          >
            &#9671;
          </div>
        ...
        <>
          <dl className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3.5">
              <dt className="text-[9px] font-bold uppercase tracking-wider text-emerald-300">
                Committed
              </dt>
              <dd className="tabular mt-1 text-lg font-semibold text-emerald-100">
      ...
      <div className="mt-1.5 flex items-center gap-2 text-[9px] text-neutral-500">
        ...
        {position.settledAt && (
          <span className="ml-auto">
            Settled {new Date(position.settledAt).toLocaleDateString()}
          </span>
        )}
      </div>
    </div>
