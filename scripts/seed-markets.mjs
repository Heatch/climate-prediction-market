...
function buildCreateMarket(protocol, authority, market, vault, marketId, questionHash, closeTimestamp, resolutionTimestamp) {
  ...
  const data = Buffer.concat([disc, marketIdBuf, hashBuf, closeBuf, resolutionBuf]);
  const keys = [
    { pubkey: protocol, isSigner: false, isWritable: true },
...
function buildFundMarket(protocol, market, vault, yesPosition, noPosition, funder, yesAmount, noAmount) {
...
const marketQuestions = {
  1001: "Will Florida record a Category 4+ hurricane landfall before October 31, 2026?",
  1002: "Will California enter severe drought conditions before December 31, 2026?",
  1003: "Did southern Ontario rainfall exceed its June historical average in 2026?",
  1004: "Will Paris, Madrid, or Rome record 40\u00B0C before September 30, 2026?",
  1005: "Will the Jamuna River cross the stated danger level before November 30, 2026?",
  1006: "Will Kenya's sample maize-yield index fall below 1.8 t/ha for the 2026 season?",
  1007: "Will Victoria declare a Catastrophic fire-danger day before March 31, 2027?",
  1008: "Was Manaus dry-season rainfall below the sample threshold by May 31, 2026?",
  1009: "Will the sample Rotterdam water-level threshold be exceeded before August 31, 2026?",
  1010: "Will any Texas station record 110\u00B0F (43.3\u00B0C) before August 5, 2026?",
  1011: "Will Oregon record a 100,000+ acre wildfire before October 15, 2026?",
  1012: "Will the Argentine Pampas enter severe drought before December 31, 2026?",
  1013: "Will Peru declare a coastal El Ni\u00F1o before February 28, 2027?",
  1014: "Will Andalusia be in meteorological drought before November 30, 2026?",
  1015: "Will Attica record a 5,000+ hectare wildfire before August 14, 2026?",
  1016: "Will the western Sahel reach IPC Crisis levels before November 30, 2026?",
  1017: "Will the Horn of Africa see a below-average rainy season by January 31, 2027?",
  1018: "Will Cape Town dam levels fall below 30% before March 31, 2027?",
  1019: "Will Maharashtra record above-normal monsoon rainfall by September 30, 2026?",
  1020: "Will a typhoon make landfall on Honshu before October 31, 2026?",
  1021: "Will the middle Yangtze exceed its warning level before August 9, 2026?",
  1022: "Will a Signal No. 3+ typhoon hit Luzon before December 31, 2026?",
  1023: "Will a tropical cyclone cross the Queensland coast before March 31, 2027?",
  1024: "Will a Wellington-area station record 35\u00B0C before February 28, 2027?",
};
...
async function main() {
  ...
  if (!protocolInfo) {
    ...
    const ix = buildInitializeProtocol(protocolPda, wallet.publicKey, wallet.publicKey);
  ...
  for (let marketId = 1001; marketId <= 1024; marketId++) {
    ...
    if (!marketInfo) {
      ...
      const createIx = buildCreateMarket(
        protocolPda, wallet.publicKey, market, vault,
        marketId, questionHash, closeTs, resolutionTs
      );
      ...
      const fundIx = buildFundMarket(
        protocolPda, market, vault,
        yesPosition, noPosition,
        wallet.publicKey,
        fundingPerSide, fundingPerSide
      );
